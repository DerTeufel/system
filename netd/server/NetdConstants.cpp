...
/*
 * merge system_nosh for being compatible with MR1
 */
int system_nosh(const char *command)
{
    ...
    argp[i] = NULL;
	ALOGI("system_nosh android_fork_execvp enter");
    res = android_fork_execvp(i, (char **)argp, NULL, false, false);
	ALOGI("system_nosh android_fork_execvp done");
	return res;
}
...
int displayRoutes(){
	...
	ALOGI(" ip    displayRoutes rule start+++++++++");
	ret = android_fork_execvp(ARRAY_SIZE(rule_cmd), (char **)rule_cmd, NULL, false, true);
        ALOGI(" ip    displayRoutes rule done----------");

        ALOGI(" ip    displayRoutes start+++++++++");
        ret = android_fork_execvp(ARRAY_SIZE(ip_cmd), (char **)ip_cmd, NULL, false, true);
        ALOGI(" ip    displayRoutes done----------");
  return ret;
}
