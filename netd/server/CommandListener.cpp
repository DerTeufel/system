...
PerfController *CommandListener::sPerfCtrl = NULL;
...
CommandListener::CommandListener() :
    ...
    /* perfController api initialize */
    sPerfCtrl->load_PerfService();

    /* lauch throughput monitor*/
    if(!sPerfCtrl->is_eng())
        sThroughputMonitor->start();
}
...
int CommandListener::BandwidthControlCmd::runCommand(SocketClient *cli, int argc, char **argv) {
    ...
    if((sPerfCtrl->get_lowpower_perfhandle() != -1) && (sPerfCtrl->get_tether_perfhandle() == -1)) {
