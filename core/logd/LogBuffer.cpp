...
int LogBuffer::log(log_id_t log_id, log_time realtime,
    ...
    if (diff_count >= DEBUG_DETECT_LOG_COUNT) {
         kernel_log_print("logd:diff time %ld, run time  %ld, count %d,step 1 %d,2 %d,3 %d,4 %d. filter count %d.\n",
             ...
             filter_count);
          // kernel_log_print("logd timout: main log size %d, radio %d,event %d, system %d, kernel %d.\n",
         // stats.sizes(LOG_ID_MAIN), stats.sizes(LOG_ID_RADIO), stats.sizes(LOG_ID_EVENTS), stats.sizes(LOG_ID_SYSTEM), stats.sizes(LOG_ID_KERNEL));
#if defined(HAVE_AEE_FEATURE) && defined(MTK_LOGD_DEBUG)
