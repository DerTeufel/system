...
static void *output_thread(void *_t)
{
...
#if !ADB_HOST
    ADBLOG("%s: starting transport output thread on fd %d, SYNC online (%d)\n",
    ...
    if(write_packet(t->fd, t->serial, &p)) {
        put_apacket(p);
#if !ADB_HOST
        ADBLOG("%s: failed to write SYNC packet\n", t->serial);
#endif
    ...
    for(;;) {
        ...
        if(t->read_from_remote(p, t) == 0){
            ...
            if(write_packet(t->fd, t->serial, &p)){
                put_apacket(p);
#if !ADB_HOST
                ADBLOG("%s: failed to write apacket to transport\n", t->serial);
#endif
...
#if !ADB_HOST
            ADBLOG("%s: remote read failed for transport\n", t->serial);
#endif
...
oops:
#if !ADB_HOST
    ADBLOG("%s: transport output thread is exiting\n", t->serial);
#endif
...
static void *input_thread(void *_t)
{
...
#if !ADB_HOST
    ADBLOG("%s: starting transport input thread, reading from fd %d\n",
       t->serial, t->fd);
#endif
    ...
    for(;;){
...
#if !ADB_HOST
            ADBLOG("%s: failed to read apacket from transport on fd %d\n",
               t->serial, t->fd);
#endif
        ...
        if(p->msg.command == A_SYNC){
            if(p->msg.arg0 == 0) {
#if !ADB_HOST
                ADBLOG("%s: transport SYNC offline\n", t->serial);
#endif
...
#if !ADB_HOST
                    ADBLOG("%s: transport SYNC online\n", t->serial);
#endif
                    D("%s: transport SYNC online\n", t->serial);
                    active = 1;
                } else {
#if !ADB_HOST
                    ADBLOG("%s: transport ignoring SYNC %d != %d\n",
...
#if !ADB_HOST
                ADBLOG("%s: transport ignoring packet while offline\n", t->serial);
#endif
...
#if !ADB_HOST
    ADBLOG("%s: transport input thread is exiting, fd %d\n", t->serial, t->fd);
#endif
...
#if !ADB_HOST
        ADBLOG("check_header(): invalid magic\n");
#endif
        return -1;
    }
...
#if !ADB_HOST
        ADBLOG("check_header(): %d > MAX_PAYLOAD\n", p->msg.data_length);
#endif
        return -1;
    }
