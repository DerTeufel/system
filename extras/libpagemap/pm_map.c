...
/* Related hardware identifiers (to be extended) */
#define KERNEL_ARCH64   "aarch64"
#define KERNEL_ARMV8   "armv8"
static int kernel_is_64bit = -1;

/* Query whether kernel is 64-bit (to be extended) */
void pm_query_is_kernel64(void) {
    struct utsname name;

    /* We have initialized kernel_is_64bit */
    ...
    /* Match KERNEL_ARCH64 */
    if (strncmp(KERNEL_ARCH64, name.machine, sizeof(KERNEL_ARCH64) - 1) == 0) {
            kernel_is_64bit = 1;
            return;
    }

    /* Match KERNEL_ARMV8 */
    if (strncmp(KERNEL_ARMV8, name.machine, sizeof(KERNEL_ARMV8) - 1) == 0) {
            kernel_is_64bit = 1;
            return;
    }
