...
#if (MTK_SSP_DEBUG_MODE_HAL_CONFIG == TRUE)
#include "hci/include/hci_config_opt_packet.h"
#endif/* MTK_SSP_DEBUG_MODE_HAL_CONFIG */
...
#if (MTK_SSP_DEBUG_MODE_HAL_CONFIG == TRUE) || (MTK_SSP_DEBUG_MODE_STACK_CONFIG == TRUE)
static bool simple_pairing_debug_mode_enabled;
#endif /* MTK_SSP_DEBUG_MODE_HAL_CONFIG || MTK_SSP_DEBUG_MODE_STACK_CONFIG */
...
static future_t *start_up(void) {
...
#if (MTK_SSP_DEBUG_MODE_HAL_CONFIG == TRUE) || (MTK_SSP_DEBUG_MODE_STACK_CONFIG == TRUE)
#if (MTK_SSP_DEBUG_MODE_HAL_CONFIG == TRUE)
  if(hci_config_opt_packet_get_interface()->get_hci_opt_packet_state(HCI_OPT_PACKET_SSP_DEBUG_MODE))
    simple_pairing_debug_mode_enabled = true;
#endif/* MTK_SSP_DEBUG_MODE_HAL_CONFIG */
#if (MTK_SSP_DEBUG_MODE_STACK_CONFIG == TRUE)
  if(stack_config_hci_opt_packet_get_interface()->get_hci_opt_packet_ssp_debug_mode_enabled())
    simple_pairing_debug_mode_enabled = true;
#endif/* MTK_SSP_DEBUG_MODE_STACK_CONFIG */
  LOG_INFO("%s: MTK_CONFIG_SSP_DEBUG_MODE is on, enabled=%d", __func__, simple_pairing_debug_mode_enabled);
  // only emit it when ssp debug mode is true
  //  the command may cost 1s
  if(simple_pairing_debug_mode_enabled){
    response = AWAIT_COMMAND(packet_factory->make_write_simple_pairing_debug_mode((simple_pairing_debug_mode_enabled ? HCI_SPD_MODE_ENABLED: HCI_SPD_MODE_DISABLED)));
    packet_parser->parse_generic_command_complete(response);
  }
#endif/* MTK_SSP_DEBUG_MODE_HAL_CONFIG || MTK_SSP_DEBUG_MODE_STACK_CONFIG */
...
#if MTK_STACK_CONFIG_LOG == TRUE
  {
    ...
    if (p_sconf_fwlog_hex->get_pack_hexlists()) {
      ...
      {
        p_hexcmd = p_hexary + j*MTK_STACK_CONFIG_NUM_OF_HEXITEMS;

        if (p_hexcmd[0] == 0)
          break;

        param_len = p_hexcmd[3];
        LOG_INFO("%s M_BTCONF hci tx c%d: opcode 0x%02x / param len %d", __func__, j, *(uint16_t *)(p_hexcmd + 1), param_len);

        for (k = 0; k < param_len; k++) {
          LOG_INFO("%02x", *(p_hexcmd + 4 + k));
        }

        response = AWAIT_COMMAND(packet_factory->make_set_cmd_from_ext(*(uint16_t *)(p_hexcmd + 1), p_hexcmd + 4, p_hexcmd[3]));
        LOG_INFO("M_BTCONF hci rx: 0x%02x 0x%02x 0x%02x 0x%02x", response->event, response->len, response->offset, response->layer_specific);
        for (i = 0; i < response->len; i++)
          LOG_INFO("%02x", response->data[i]);
