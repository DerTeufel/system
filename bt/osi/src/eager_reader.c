...
static void inbound_data_waiting(void *context) {
  ...
  if (bytes_read > 0) {
    ...
    if (bytes_read < 0) {
      if (ERRCODE_CHIP_RESET_IS_DOING == errno) {
        LOG_ERROR("%s doing chip-reset", __func__);
        reader->detect_chiprst_in_read = 1;
      }
      else if (ERRCODE_CHIP_RESET_IS_DONE == errno) {
...
#if MTK_HCITRANS_DETECT_CHIP_RESET == TRUE
bool eager_reader_check_chiprst(eager_reader_t *reader)
{
  return reader->detect_chiprst_in_read;
}
#endif
