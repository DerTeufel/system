...
void bta_dm_acl_change(tBTA_DM_MSG *p_data)
{
    ...
    if(is_new)
    {
        ...
        if (((NULL != (p = BTM_ReadLocalFeatures ())) && HCI_SNIFF_SUB_RATE_SUPPORTED(p)) &&
             ...
             (!bta_dm_mtk_is_device_blacklisted(p_bda, stack_config_sniff_subrating_blacklist_get_interface())) &&
#endif
...
#if defined(MTK_LEGACY_B3DS_SHORTEN_EIR) && (MTK_LEGACY_B3DS_SHORTEN_EIR == TRUE)
...
{
#if MTK_STACK_CONFIG_BL == TRUE
    if (!BTM_SecAddBleDevice (p_data->add_ble_device.bd_addr, p_data->add_ble_device.bd_name,
#else
