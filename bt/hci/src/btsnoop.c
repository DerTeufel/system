...
#include "twrite.h"
#endif
#if MTK_BTSNOOPLOG_NAME_READABLE == TRUE
#include <time.h>
#endif
...
#if MTK_BTSNOOPLOG_THREAD == TRUE
static TWRITE_FD *logthread_fd = NULL;
#endif
...
static void update_logging() {
  ...
  if (should_log) {
    ...
    {
      struct stat st = {0};
      char log_dir[MTK_STACK_CONFIG_FPATH_LEN], tmp[MTK_STACK_CONFIG_FPATH_LEN];
      int path_len = strlen(log_path);

      LOG_INFO("%s M_BTCONF log_path: %s", __func__, log_path);
      if (0 < path_len && path_len < MTK_STACK_CONFIG_FPATH_LEN) {
        ...
        while(log_dir[i])
        {
          ...
          {
            tmp[i] = '\0';
            LOG_INFO("%s tmp: %s", __func__, tmp);
            if(access(tmp, F_OK) != 0)
            {
              if(mkdir(tmp, 0770) == -1)
              {
                LOG_ERROR("mkdir error! %s",(char*)strerror(errno));
                break;
              }
            }
            tmp[i] = '/';
        ...
        LOG_ERROR("%s M_BTCONF log_path is longer then 1024", __func__);
      }
    }
#endif
    ...
    if (stack_config->get_btsnoop_should_save_last()) {
...
#if MTK_BTSNOOPLOG_NAME_READABLE == TRUE
      snprintf(last_log_path, PATH_MAX, "%s.%llu", log_path, readability_timestamp());
#else
      ...
      if (!rename(log_path, last_log_path) && errno != ENOENT)
        ...
        LOG_INFO("%s rename previous btsnoop_hci.log to %s sucessfully!", __func__, last_log_path);
    }
...
#if MTK_BTSNOOPLOG_THREAD == TRUE
    logthread_fd = twrite_init(logfile_fd);
    if (logthread_fd != NULL) {
      LOG_INFO("%s twrite log thread is started!", __func__);

      int nWritten = write(logfile_fd, "btsnoop\0\0\0\0\1\0\0\x3\xea", 16);
      if (nWritten != 16) {
        LOG_INFO("write failed : nWritten=%d", nWritten);

        if (nWritten < 0)
          LOG_ERROR("write failed: err=%s, errno=%d", strerror(errno), errno);

        if (logthread_fd != NULL)
          twrite_deinit(logthread_fd);
        logthread_fd = NULL;
        ...
        LOG_ERROR("There is no hci log in this run!");
      }
    } else {
        LOG_INFO("%s create twrite log thread fails, there's no hci log!", __func__);
    }
#else
...
#if MTK_BTSNOOPLOG_THREAD == TRUE
  if (logthread_fd != NULL)
    if ((int)length != twrite_write(logthread_fd, data, length))
      LOG_ERROR("%s twrite_write data failed", __func__);
#else
  if (logfile_fd != INVALID_FD)
...
#if MTK_BTSNOOPLOG_THREAD == TRUE
  /* pack multiple writes to reduce the time of invoking btsnoop_write() function */
  uint8_t acc_header[32] = {'\0'};

  memcpy(acc_header, &length, 4);
