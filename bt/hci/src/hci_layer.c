...
static void command_timed_out(UNUSED_ATTR void *context) {
...
#if MTK_HCITRANS_DETECT_CHIP_RESET == TRUE
  if (true == hal->is_chip_doing_reset()) {
    LOG_INFO("%s Bypass command_timed_out() because \"whole-chip-reset\" is triggered!", __func__);
    return;
  }
#endif

#if MTK_FWLOG_TRIG_COREDUMP == TRUE
  /* It may trigger coredump when chip is not resetting */
  /* & Make sure trigger coredump once */
  if (true == b_trig_coredump) {
    LOG_INFO("%s Bypass command_timed_out() because \"fw-coredump\" is triggered!", __func__);
    return;
  }
#endif
   ...
   */
  LOG_INFO("%s Stack triggers firmware coredump!", __func__);
  stack_trigger_reason = 31; /* 31 for host assert */
  if (0 == vendor->send_command(VENDOR_SET_FW_ASSERT, &stack_trigger_reason)) {
