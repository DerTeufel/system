...
static BT_HDR *make_write_simple_pairing_debug_mode(uint8_t mode) {
  uint8_t *stream;
  const uint8_t parameter_size = 1;
  BT_HDR *packet = make_command(HCI_WRITE_SIMP_PAIR_DEBUG_MODE, parameter_size, &stream);
...
#if (MTK_SSP_DEBUG_MODE_HAL_CONFIG == TRUE) || (MTK_SSP_DEBUG_MODE_STACK_CONFIG == TRUE)
  make_write_simple_pairing_debug_mode,
#endif /* MTK_SSP_DEBUG_MODE_HAL_CONFIG || MTK_SSP_DEBUG_MODE_STACK_CONFIG */
  make_write_secure_connections_host_support,
  make_set_event_mask,
  make_set_event_mask_2,    /* MTK_B3DS_INCLUDED */
#if MTK_STACK_CONFIG_LOG == TRUE
