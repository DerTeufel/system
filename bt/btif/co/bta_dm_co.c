...
#include <string.h>
extern BOOLEAN bta_dm_mtk_is_device_blacklisted(BD_ADDR remote_bd_addr,const stack_config_blacklist_t * blacklist);
#endif
...
#if (MTK_STACK_CONFIG_BL == TRUE)
        if(bta_dm_mtk_is_device_blacklisted(bd_addr, stack_config_secure_connections_blacklist_interface()))
        {
            *p_auth_req &= ~BTA_LE_AUTH_REQ_SC_ONLY;
            BTIF_TRACE_DEBUG("bta_dm_co_ble_io_req: IOT device, disable security connections, auth_req: %d", *p_auth_req);
        }
#endif
...
#if MTK_STACK_CONFIG_BL == TRUE
BOOLEAN bta_dm_mtk_is_device_blacklisted(BD_ADDR remote_bd_addr, const stack_config_blacklist_t *blacklist)
{
    char *token;
    char bdstr[18];
    char *dev_name_str;
    const char *value;
    char partial_name[MTK_STACK_CONFIG_PARTIAL_NAME_VALUE_MAX];

    if(blacklist == NULL || remote_bd_addr == NULL)
        return FALSE;
    ...
    if((blacklist->get_addr_blacklist != NULL) && (value = blacklist->get_addr_blacklist()))
    {
        APPL_TRACE_EVENT("bta_dm_mtk_is_device_blacklisted: check %s in %s", bdstr, value);
        if (strcasestr(value,bdstr) != NULL)
            return TRUE;
    }

    dev_name_str = BTM_SecReadDevName(remote_bd_addr);
    if (dev_name_str == NULL || dev_name_str[0] == '\0')
    {
        tBTM_INQ_INFO       *p_inq_info = BTM_InqDbRead(remote_bd_addr);
        if (p_inq_info != NULL)
        {
            dev_name_str = (char *)&p_inq_info->remote_name;
        }
    }

    if (dev_name_str != NULL && dev_name_str[0] != '\0')
    {
        if((blacklist->get_name_blacklist != NULL) && (value = blacklist->get_name_blacklist()))
        {
            APPL_TRACE_EVENT("bta_dm_mtk_is_device_blacklisted: check %s in %s", dev_name_str, value);
            if (strstr(value,dev_name_str) != NULL)
                return TRUE;
        }
        if((blacklist->get_partial_name_blacklist != NULL) && (value = blacklist->get_partial_name_blacklist()))
        {
            strncpy(partial_name, value, MTK_STACK_CONFIG_PARTIAL_NAME_VALUE_MAX);
            APPL_TRACE_EVENT("bta_dm_mtk_is_device_blacklisted: check %s in %s", partial_name, dev_name_str);
            token = strtok(partial_name, MTK_STACK_CONFIG_VALUE_SEPARATOR);
            while (token != NULL)
            {
                if (strstr(dev_name_str, token) != NULL)
                    return TRUE;

                token = strtok(NULL, MTK_STACK_CONFIG_VALUE_SEPARATOR);
            }
        }
    }
    return FALSE;
...
BOOLEAN bta_dm_mtk_is_device_blacklisted2(BD_ADDR remote_bd_addr, const stack_config_blacklist_t *blacklist)
{
    char bdstr[18];
    char *dev_name_str;
    const char *value;
    BOOLEAN name_pass = FALSE;
    BOOLEAN addr_pass = FALSE;
    ...
    if((blacklist->get_addr_blacklist != NULL) && (value = blacklist->get_addr_blacklist()))
    {
        APPL_TRACE_EVENT("bta_dm_mtk_is_device_blacklisted2: addr check %s in %s", bdstr, value);
        if (strcasestr(value,bdstr) != NULL) {
            addr_pass = TRUE;
        }
    }
    ...
    if (dev_name_str != NULL && dev_name_str[0] != '\0')
    {
        if((blacklist->get_name_blacklist != NULL) && (value = blacklist->get_name_blacklist()))
        {
            APPL_TRACE_EVENT("bta_dm_mtk_is_device_blacklisted2: name check %s in %s", dev_name_str, value);
            if (strstr(value,dev_name_str) != NULL) {
                name_pass = TRUE;
            }
        }
    }
    if (addr_pass == TRUE && name_pass == TRUE) {
        return TRUE;
    }
    return FALSE;
}
