...

{
    ...
    case NON_A2DP_MEDIA_CT:
        ...
        {
            supported = TRUE;
            APPL_TRACE_ERROR("[bta_av_co_audio_getconfig] APTX");
        }
        break;
#endif
#if (MTK_A2DP_SRC_AAC_CODEC == TRUE)
...
#if (MTK_STACK_CONFIG_BL == TRUE)
                    && (!bta_dm_mtk_is_device_blacklisted(p_peer->addr, stack_config_scms_t_blacklist_get_interface()))
#endif
...
#if (MTK_STACK_CONFIG_BL == TRUE)
                    if ((cp_active == TRUE) && (bta_dm_mtk_is_device_blacklisted(p_peer->addr, stack_config_scms_t_blacklist_get_interface()))){
                        cp_active = FALSE;
...
BOOLEAN bta_av_co_audio_get_aac_aptx_config(UINT8 *p_config, UINT16 *p_minmtu, UINT8 type)
{
    ...
    *p_minmtu = 0xFFFF;
    ...
    for (int index = 0; index < BTA_AV_CO_NUM_ELEMENTS(bta_av_co_cb.peers); index++)
    {
        p_peer = &bta_av_co_cb.peers[index];
        if (p_peer->opened)
        {
