...
/*******************************************************************************
**
** Function         btif_storage_get_ble_device_name
**
** Description      BTIF storage API - Fetches the remote device name, currently only
                       used for mtk blacklist
**
** Returns          BT_STATUS_SUCCESS if the fetch was successful,
**                  BT_STATUS_FAIL otherwise
**
*******************************************************************************/
bt_status_t btif_storage_get_ble_device_name(bt_bdaddr_t *remote_bd_addr,
                                              BD_NAME *bd_name)
{
    bdstr_t bdstr;
    int length = BD_NAME_LEN;
    bdaddr_to_string(remote_bd_addr, bdstr, sizeof(bdstr));
    int ret = btif_config_get_str(bdstr, "Name", bd_name, &length);
    return ret ? BT_STATUS_SUCCESS : BT_STATUS_FAIL;
}
#endif
#endif
