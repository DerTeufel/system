...
static void btif_media_task_aa_start_tx(void)
{
    ...
    if(codecType == NON_A2DP_MEDIA_CT)
    {
        alarm_set_periodic(btif_media_cb.media_alarm, BTIF_MEDIA_TIME_TICK /MTK_A2DP_BTIF_MEDIA_TIME_EXT , btif_media_task_alarm_cb, NULL);
    }
    else
#endif
    {
...
static void btif_media_task_aa_stop_tx(void)
{
...
#if (MTK_COMMON == TRUE)
    bool send_ack = (btif_media_cb.is_tx_timer != FALSE);
#else
    ...
    /* Flash the TxAaQ again after A2DP suspended */
#if (MTK_COMMON == TRUE)
...
static UINT8 btif_get_num_aa_frame(void)
{
    ...
    switch (btif_media_cb.TxTranscoding)
    {
        ...
        case BTIF_MEDIA_TRSCD_PCM_2_APTX:
            ...
            {
                 result = BTIF_MEDIA_FR_PER_TICKS_44_1_APTX;
            }
            else
            {
                result = BTIF_MEDIA_FR_PER_TICKS_44_1_APTX - 1;
            }
            break;
#endif
#if (MTK_A2DP_SRC_AAC_CODEC == TRUE)
        case BTIF_MEDIA_TRSCD_PCM_2_AAC:
            {
                result = MTK_A2DP_BTIF_MEDIA_FR_PER_TICKS_AAC;
                break;
            }
#endif
...
static void btif_media_aa_prep_aptx_2_send(UINT8 nb_frame)
{
    ...
    while(nb_frame)
    {
        ...
        {
            APPL_TRACE_ERROR ("ERROR btif_media_aa_prep_aptx_2_send no buffer TxCnt %d ", btif_media_cb.TxAaQ._count);
            return;
        }
        ...
        }  while (((p_buf->len + btif_media_cb.aptxEncoderParams.u16PacketLength) < btif_media_cb.TxAaMtuSize)
                  && (p_buf->layer_specific < 0x07) && nb_frame);

        LOG_VERBOSE("TX QUEUE NOW %d", btif_media_cb.TxAaQ._count);
        if (btif_media_cb.tx_flush)
        {
            APPL_TRACE_DEBUG("### tx suspended, discarded frame ###");
            if (btif_media_cb.TxAaQ._count > 0)
                btif_media_flush_q(&(btif_media_cb.TxAaQ));
            GKI_freebuf(p_buf);
            return;
        }
...
static void btif_media_aa_prep_aac_2_send(UINT8 nb_frame)
{
    ...
    for(; current_buf_size >= MTK_A2DP_AAC_ENC_INPUT_BUF_SIZE; current_buf_size -= MTK_A2DP_AAC_ENC_INPUT_BUF_SIZE)
    {
        if (NULL == (p_buf = GKI_getpoolbuf(BTIF_MEDIA_AA_POOL_ID)))
        {
            APPL_TRACE_ERROR ("[ERROR] btif_media_aa_prep_aac_2_send no buffer TxCnt %d ", btif_media_cb.TxAaQ._count);
            return;
        }
        ...
        int inBufferSize[] = {MTK_A2DP_AAC_ENC_INPUT_BUF_SIZE / 2};
        ...
        if (AACENC_OK == (errNumber = aacEncEncode(btif_media_cb.aacEncoderParams.aacEncoder, &inputBufDesc, &outputBufDesc, &inArgs, &outArgs)))
        {
            ...
            APPL_TRACE_VERBOSE("[AAC] TX QUEUE NOW %d", btif_media_cb.TxAaQ._count);
...
void btif_media_av_delay_start_cmd_hdlr(TIMER_LIST_ENT *tle)
{
