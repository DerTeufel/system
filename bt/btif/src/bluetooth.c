...
static int config_ssp_debug_mode(bool enable){
#if MTK_SSP_DEBUG_MODE_HAL_CONFIG  == TRUE
    LOG_INFO("%s: enable=%d", __func__, enable);
    if (!interface_ready())
        return BT_STATUS_NOT_READY;
    hci_config_opt_packet_get_interface()->set_hci_opt_packet_state(HCI_OPT_PACKET_SSP_DEBUG_MODE, enable);
    return BT_STATUS_SUCCESS;
#else
