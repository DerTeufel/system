...
#if (!defined(MTK_B3DS_FIXED_COD)) || (MTK_B3DS_FIXED_COD == FALSE)
  // Set class of device.
  tBTA_UTL_COD cod;
...
error:
    SDP_DeleteRecord(handle);
    APPL_TRACE_ERROR("add_spp_sdp: failed to register SPP service, stage: %s, service_name: %s", stage, name);
    return 0;
}
#endif //MTK_DUN_GW_12 == TRUE
