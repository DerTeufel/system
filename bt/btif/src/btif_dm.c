...
/* fetch additional info about remote device used in iop query */
 ...
 /* if not available yet, try fetching from config database */
    ...
    for (unsigned int i = 0; i < MAX_SDP_BL_ENTRIES && i < sizeof(sdp_blacklist)/sizeof(skip_sdp_entry_t); i++)
    {
...
#if (MTK_STACK_CONFIG_BL == TRUE)
    if(bta_dm_mtk_is_device_blacklisted(*(BD_ADDR*)remote_bdaddr, stack_config_hid_disable_sdp_blacklist_get_interface()))
        return TRUE;
#endif
...
static void btif_dm_auth_cmpl_evt (tBTA_DM_AUTH_CMPL *p_auth_cmpl)
{
    ...
    if (p_auth_cmpl->success)
    {
...
#endif
        {
            ...
            LOG_WARN("%s: blacklist device found!! skip SDP",__FUNCTION__);
            bt_property_t prop;
            bt_uuid_t  uuid;
