...
const BOOLEAN bta_dm_mtk_is_device_blacklisted(BD_ADDR remote_bd_addr, const stack_config_blacklist_t *blacklist);
#endif
...
#if MTK_STACK_CONFIG_BL == TRUE
#define BTIF_TIMER_A2DP_DELAY_START_CMD  1
#endif
...
#if MTK_STACK_CONFIG_BL == TRUE
static TIMER_LIST_ENT tle_av_delay_start_cmd;
#endif
...
static BOOLEAN btif_av_state_opening_handler(btif_sm_event_t event, void *p_data)
{
    ...
    switch (event)
    {
        ...
        case BTIF_AV_DISCONNECT_REQ_EVT:
             ...
             BTA_AvDisconnect((bt_bdaddr_t*)p_data);
            break;
#endif
...
{
    const uint8_t *ptr = btif_av_cb.peer_bda.address;
    bdstr_t string;
    sprintf(string, "%02x:%02x:%02x:%02x:%02x:%02x",
             ptr[0], ptr[1], ptr[2],
             ptr[3], ptr[4], ptr[5]);
    BTIF_TRACE_DEBUG("%s string : %s", __func__, string);

    if (sizeof(btif_av_cb.peer_bda) != 0) {
        if(bta_dm_mtk_is_device_blacklisted(btif_av_cb.peer_bda.address,
                stack_config_a2dp_delay_start_cmd_blacklist_get_interface())) {
            memset(&tle_av_delay_start_cmd, 0, sizeof(tle_av_delay_start_cmd));
            tle_av_delay_start_cmd.param = (UINT32)btif_media_av_delay_start_cmd_hdlr;
            btu_start_timer(&tle_av_delay_start_cmd, BTU_TTYPE_USER_FUNC,
