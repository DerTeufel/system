...
{
        mtk_btif_hf_features |= BTIF_HF_FEATURES;
...
/*******************************************************************************
**
** Function         btif_hf_is_call_exist
**
** Description      check if the call still exist when create the SCO.
**
** Returns          bool
**
*******************************************************************************/

bool btif_hf_is_call_exist() {
    int idx;
    if (hf_idx != BTIF_HF_INVALID_IDX) {
        idx = hf_idx;
    } else {
    ...
    if(btif_hf_cb[idx].num_active == 0 && btif_hf_cb[idx].num_held == 0
            && btif_hf_cb[idx].call_setup_state == BTHF_CALL_STATE_IDLE) {
        return FALSE;
    }
    return TRUE;
