                               ...
                               UINT16 conn_interval, UINT16 conn_latency, UINT16 conn_timeout)
{
...
#if MTK_STACK_CONFIG_BL == TRUE
    tBTM_INQ_INFO       *p_inq_info = BTM_InqDbRead(bda);

    if(p_inq_info)
    {
        memset(p_dev_rec->sec_bd_name, sizeof(p_dev_rec->sec_bd_name), 0);
        memcpy(p_dev_rec->sec_bd_name, p_inq_info->remote_name, p_inq_info->remote_name_len);
    }
#endif
          ...
          (conn_timeout > p_dev_rec->conn_params.supervision_tout)))
    {
        ...
        btsnd_hcic_ble_upd_ll_conn_params (handle,
                                           ...
                                           MTK_BLE_CONN_MIN_CE_LEN, MTK_BLE_CONN_MAX_CE_LEN);
#else
                                           0, 0);
#endif
    }
...
static void l2cble_start_conn_update (tL2C_LCB *p_lcb)
{
    ...
    else
    {
        ...
        {
#if (MTK_STACK_CONFIG_BL == TRUE)
            if(bta_dm_mtk_is_device_blacklisted(p_lcb->remote_bd_addr, stack_config_ble_conn_timeout_blacklist_get_interface()))
            {
            ...
            if (p_lcb->link_role == HCI_ROLE_MASTER)
            {
                    ...
                    max_interval < min_interval)
                {
                    ...
                    if((max_interval < BTM_BLE_CONN_INT_MIN_LIMIT)
                        && bta_dm_mtk_is_device_blacklisted(p_lcb->remote_bd_addr, stack_config_ble_conn_int_min_limit_blacklist_get_interface()))
                    {
#if defined(MTK_BLE_CONN_INT_MIN_LIMIT) && defined(MTK_BLE_CONN_INT_MAX_LIMIT)
                        if(p_lcb->min_interval != MTK_BLE_CONN_INT_MIN_LIMIT || p_lcb->max_interval != MTK_BLE_CONN_INT_MAX_LIMIT)
                        {
                            p_lcb->min_interval = MTK_BLE_CONN_INT_MIN_LIMIT;
                            p_lcb->max_interval = MTK_BLE_CONN_INT_MAX_LIMIT;
                            p_lcb->conn_update_mask |= L2C_BLE_NEW_CONN_PARAM;
...
#if MTK_STACK_CONFIG_BL == TRUE
                    if(bta_dm_mtk_is_device_blacklisted(p_lcb->remote_bd_addr, stack_config_hogp_conn_update_blacklist_get_interface()))
                    {
                        l2cu_send_peer_ble_par_rsp (p_lcb, L2CAP_CFG_UNACCEPTABLE_PARAMS, id);
