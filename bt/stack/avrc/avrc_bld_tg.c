...
**
** Description      This function builds the set addressed player response.
**
** Returns          AVRC_STS_NO_ERROR, if the response is built successfully
**                  Otherwise, the error code.
**
...
static tAVRC_STS avrc_bld_set_addr_player_rsp( tAVRC_RSP *p_rsp, BT_HDR *p_pkt )
{
    ...
    AVRC_TRACE_API("avrc_bld_set_addr_player_rsp: status=%d, pdu:x%x", p_rsp->status, p_rsp->pdu);
...
{
    UINT8 *p_data, *p_start, *p_item_start;
    UINT16 count, len;
    int i = 0;
    UINT8 type = AVRC_ITEM_PLAYER;

    AVRC_TRACE_API("avrc_bld_get_folder_items_rsp: status=%d, pdu:x%x", p_rsp->status, p_rsp->pdu);
    ...
    count = p_rsp->item_count;
    p_item_start = p_data;

    while(count != 0) {
        type = p_rsp->p_item_list[i].item_type;
        switch (type) {
            case AVRC_ITEM_PLAYER:
                ...
                ARRAY_TO_BE_STREAM(p_data,p_rsp->p_item_list[i].u.player.name.p_str, p_rsp->p_item_list[i].u.player.name.str_len);
                len = (p_data - p_item_start - 3);
                p_item_start++;
                UINT16_TO_BE_STREAM(p_item_start,len);
                p_item_start = p_data;
                break;
            default:
                p_rsp->item_count--;
                AVRC_TRACE_WARNING("avrc_bld_get_folder_item_rsp: no support this item type");
                break;
            }
        i++;
        count--;
    ...
    p_pkt->len = (p_data - p_start);
    p_data = p_start + 1;
    UINT16_TO_BE_STREAM(p_data,p_pkt->len - 3);
...
/*******************************************************************************
**
** Function         avrc_bld_get_total_num_rsp
**
...
**
*******************************************************************************/
static tAVRC_STS avrc_bld_get_total_num_rsp( tAVRC_SEARCH_RSP*p_rsp, BT_HDR *p_pkt )
{
    UINT8 *p_start,*p_data;

    AVRC_TRACE_API("avrc_bld_get_total_num_rsp: status=%d, pdu:x%x", p_rsp->status, p_rsp->pdu);
...
/*******************************************************************************
**
** Function         avrc_bld_general_reject_rsp
**
** Description      This function builds the general reject response as PDU is unknown
**
** Returns          AVRC_STS_NO_ERROR, if the response is built successfully
**                  Otherwise, the error code.
**
*******************************************************************************/
static tAVRC_STS avrc_bld_general_reject_rsp( tAVRC_RSP *p_rsp, BT_HDR *p_pkt )
{
    UINT8 *p_data, *p_start;

    AVRC_TRACE_API("avrc_bld_general_reject_rsp: status=%d, pdu:x%x", p_rsp->status, p_rsp->pdu);
...
static tAVRC_STS avrc_bld_rejected_rsp( tAVRC_RSP *p_rsp, BT_HDR *p_pkt )
{
    ...
    if (p_pkt->layer_specific == AVCT_DATA_CTRL)
        p_data = p_start + 2;
#if (MTK_AVRCP_TG_15 == TRUE)
    else
        p_data = p_start + 1;
...
tAVRC_STS AVRC_BldResponse( UINT8 handle, tAVRC_RESPONSE *p_rsp, BT_HDR **pp_pkt)
{
    ...
    case AVRC_PDU_GET_FOLDER_ITEMS: /* 0x71 */
        ...
        break;
    case AVRC_PDU_GET_TOTAL_NUMBER_OF_ITEMS: /* 0x75 */
        status = avrc_bld_get_total_num_rsp(&p_rsp->total_num, p_pkt);
        break;
    case AVRC_PDU_GENERAL_REJECT: /* 0xa0 */
        status = avrc_bld_general_reject_rsp(&p_rsp->rsp, p_pkt);
        break;
#endif
    }
