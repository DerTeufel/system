                                      ...
                                      UINT8 *p_buf, UINT16 buf_len)
{
    ...
    case AVRC_PDU_SET_ADDRESSED_PLAYER:     /* 0x60 */
        if (len != 2) {
            status = AVRC_STS_INTERNAL_ERR;
            AVRC_TRACE_ERROR("AVRC_PDU_SET_ADDRESSED_PLAYER: wrong length");
        } else
...
#if (MTK_AVRCP_TG_15== TRUE)
static tAVRC_STS avrc_pars_browse_cmd(tAVRC_MSG_BROWSE *p_msg, tAVRC_COMMAND *p_result)
{
    tAVRC_STS  status = AVRC_STS_NO_ERROR;
    ...
    p_result->pdu = *p++;
    AVRC_TRACE_DEBUG("avrc_pars_browse_cmd() pdu:0x%x", p_result->pdu);
    ...
    switch (p_result->pdu) {
        case AVRC_PDU_GET_FOLDER_ITEMS:
            ...
            if (p_result->get_items.scope == AVRC_SCOPE_PLAYER_LIST) {
                p_result->get_items.attr_count = 0;
                AVRC_TRACE_DEBUG("Ignore specific attributes requested");
                break;
            }
            ...
            if ((count != 0x00) && (count != 0xFF)) {
                p_attr = p_result->get_items.p_attr_list;

                while (count > 0) {
                    BE_STREAM_TO_UINT32(*p_attr,p);
                    count--;
                    p_attr++;
                }
            }
        ...
        case AVRC_PDU_GET_TOTAL_NUMBER_OF_ITEMS:
            BE_STREAM_TO_UINT8(p_result->get_total_num.scope,p);
            break;
    default:
        status = AVRC_STS_BAD_CMD;
        break;
...
#if (MTK_AVRCP_TG_15== TRUE)
        case AVRC_OP_BROWSE:
            status = avrc_pars_browse_cmd(&p_msg->browse, p_result);
            break;
#endif
        default:
