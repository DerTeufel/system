...
status_t PublicVolume::doUnmount() {
    ...
    if (TEMP_FAILURE_RETRY(rmdir(mFuseDefault.c_str()))) {
        PLOG(ERROR) << getId() << " failed to rmdir mount point " << mFuseDefault;
    }
    if (TEMP_FAILURE_RETRY(rmdir(mFuseRead.c_str()))) {
        PLOG(ERROR) << getId() << " failed to rmdir mount point " << mFuseRead;
    }
    if (TEMP_FAILURE_RETRY(rmdir(mFuseWrite.c_str()))) {
        PLOG(ERROR) << getId() << " failed to rmdir mount point " << mFuseWrite;
    }
